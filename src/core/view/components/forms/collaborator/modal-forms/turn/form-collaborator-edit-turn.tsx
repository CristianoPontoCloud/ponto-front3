...
import { useFormCollaboratorEditTurn } from "./use-form-collaborator-edit-turn";

export interface FormCollaboratorEditTurn {
	collaboratorId: string;
	getSubmitResponse?: (data: WorkShiftAssignment[]) => void;
	collaboratorViewer: CollaboratorViewerParams;
}

export function FormCollaboratorEditTurn({
	collaboratorViewer,
	collaboratorId,
	getSubmitResponse,
}: FormCollaboratorEditTurn) {
	const { form, onSubmit, resetModal, cycleDaysData } = useFormCollaboratorEditTurn({
		collaboratorId,
		getSubmitResponse,
	});
	const fullspan = "col-span-2 sm:col-span-2 md:col-span-2 lg:col-span-2";
	useErrorToastSubmit<CollaboratorEditTurnFormProps>({ form });
