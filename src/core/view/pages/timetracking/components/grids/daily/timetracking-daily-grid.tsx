...
import { useTimetrakingDailyGrid } from "./use-timetracking-daily-grid";
interface TimetrackingDailyGridParams {
	dailyResponse?: MarkDailyResponse;
	isFullScreen: boolean;
	height: number;
	isLoading: boolean;
}
export default function TimetrackingDailyGrid({
	dailyResponse,
	isFullScreen,
	height,
	isLoading,
}: TimetrackingDailyGridParams) {
	const { columns, fixColumns, rows } = useTimetrakingDailyGrid({ dailyResponse });
	...
	if (!dailyResponse) return <TimetrackingGridNoResults height={height} />;
