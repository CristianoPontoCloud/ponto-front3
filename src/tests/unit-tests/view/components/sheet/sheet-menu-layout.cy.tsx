...
describe("<SheetMenuLayout />", () => {
	...
	const initialForm = {
		id: 0,
		FormComponent: () => <div>{formAContent}</div>,
	};
	...
	const formB: SheetMenuItem = {
		form: {
			id: 1,
			FormComponent: () => <div>{formBContent}</div>,
		},
		label: formBContent,
		Icon: User,
		errorMapFields: [],
	...
	function FormMock() {
		const [currentForm, setCurrentForm] = useState<SheetMenuItem["form"]>(initialForm);
		return (
			<div>
				<SheetMenuLayout
	...
	it("Should be render initial form-sheet-menu-layout", () => {
		cy.get("#form-sheet-menu-layout").should("exist");
	});
	it("Should be render initial sheet-footer-submit", () => {
		cy.get("#sheet-footer-submit").should("exist");
	});
	it("Should be render buttons with label text, when click it should change the form", () => {
		for (const { label } of menuItems) {
			...
			cy.contains("div", label).should("exist");
		}
	});
});
